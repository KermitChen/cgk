...
//        System.out.println(connectionManager.getConnection("es"));
    ...
    private static Config config = Config.getInstance();///获取配置文件信息
    private static String logFolder = config.getLogFolder();//日志文件存储路径
    String serverIp = config.getServerIp();//ES数据库集群IP集合，以半角逗号分隔
    String clusterName = config.getClusterName();//集群名称
    ...
    static synchronized public ESClientManager getInstance() {    
    	try {
			...
			StringUtil.writerTXT(logFolder, "ES数据库连接池初始化异常"+e.getMessage());
			e.printStackTrace();
		}
         return null;
    }    
    ...
    class DBConnectionPool {    
        ...
        private Client newConnection() {    
        	...
        	Settings settings = ImmutableSettings.settingsBuilder()
					...
					.build();
        	TransportClient clientTran = new TransportClient(settings);
        	try {
        		 String strIp[] = serverIp.split(",");
        		 if(strIp != null && strIp.length > 0){
        			 for(int i=0;i < strIp.length;i++){
            			 clientTran.addTransportAddresses(new InetSocketTransportAddress(strIp[i], 9300));
            		 }
        		 }else{
