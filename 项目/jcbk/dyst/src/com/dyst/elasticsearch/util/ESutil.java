...
public class ESutil {
	...
	public static FilterBuilder getFilterByCon(String kssj, String jssj,
			...
			String cd, String cb, String sd, String hmdCphm, String business, String sort) {
		...
		if (kssj != null && !"".equals(kssj.trim()) && jssj != null
			...
			try {
				filtersbsj = FilterBuilders.boolFilter().must(
						FilterBuilders.rangeFilter((sort != null && "scsj".equals(sort.trim().toLowerCase())?"scsj":"tgsj")).from(sdf.parse(kssj).getTime())
			...
			if("04".equals(business)){//模糊查询
				...
				if(hphm.startsWith("!")){//非前缀查询
					...
					if(hphm.substring(0, 1).equals(qz1) ){
						...
						filterhphm = FilterBuilders.boolFilter().must(FilterBuilders.prefixFilter("cphm1", hphm)).mustNot(FilterBuilders.prefixFilter("cphm1", qz2));
//						queryBuilder.mustNot(QueryBuilders.prefixQuery("cphm1", qz2));
		...
		} else if("11".equals(business.trim())){//不区分已识别和未识别
