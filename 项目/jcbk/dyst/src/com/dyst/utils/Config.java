...
public class Config {
	...
	private String hkPic;//海康图片地址
	private String picCall;//停车场图片调用地址
	private String kjcPic;//科技处图片
	...
	private void init() {
		...
		try {
			...
			hkPic = root.element("hkPic").getText().trim();
			picCall = root.element("picCall").getText().trim();
			kjcPic = root.element("kjcPic").getText().trim();
			username = root.element("username").getText().trim();
			token = root.element("token").getText().trim();
			...
			clusterName = root.element("clusterName").getText();// 集群名称
			...
			zkCon = root.element("zkCon").getText().trim();
			splitStr = root.element("splitStr").getText().trim();
			businessTopic = root.element("businessTopic").getText().trim();
			errorTopic = root.element("errorTopic").getText().trim();
			bulkCount = StringUtils.isEmpty(root.element("bulkCount").getText().trim())?200:Integer.parseInt(root.element("bulkCount").getText().trim());
			isInsertLog = root.element("isInsertLog").getText().trim();
	...
	public String getIsInsertLog(){
		return isInsertLog;
	}

	public String getHkPic() {
		return hkPic;
	}

	public String getKjcPic() {
		return kjcPic;
	}
