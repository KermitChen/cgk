...
Dates.timeVoid = function(times, index){
    if(Dates.ymd[1]+1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]){
        if(index === 0 && (times < Dates.mins[3])){
			...
			if(Dates.inymd[3] === Dates.mins[3] && Dates.inymd[4] === Dates.mins[4]){
				if(times < Dates.mins[5]) {
					return 1;
				}else{
					return 0;
				}
        ...
        if(index === 0 && times > Dates.maxs[3]){
			...
			if(Dates.inymd[3] === Dates.maxs[3]){
				if(times > Dates.maxs[4]) {
					return 1;
				}else{
					return 0;
				}
             ...
             if(Dates.inymd[3] === Dates.maxs[3] && Dates.inymd[4] === Dates.maxs[4]){
				if(times > Dates.maxs[5]) {
					return 1;
				}else{
					return 0;
				}
...
Dates.events = function(){
    ...
    Dates.on(S(log.box), 'click', function(){
        Dates.reshow();
        if($(".laydte_hsmtex").text().indexOf('小时')>=0){
            $("#laydate_hms input").eq(1).css("background", "#CCCCCC");
            $("#laydate_hms input").eq(0).css("background", "#fff");
            $("#laydate_hms input").eq(1).trigger('click');
            num ++;
            return;
        }
        if($(".laydte_hsmtex").text().indexOf('分钟')>=0 && num>=1){
            $("#laydate_hms input").eq(2).css("background", "#CCCCCC");
            $("#laydate_hms input").eq(1).css("background", "#fff");
            $("#laydate_hms input").eq(0).css("background", "#fff");
            $("#laydate_hms input").eq(2).trigger('click');
            num = 0;
            return;
        }
        $("#laydate_hms input").eq(2).css("background", "#fff") 
    });
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                // Dates.addClass(this, as[3])
                Dates.addClass(this, 'laydate_click');
                $(this).siblings().removeClass('laydate_click');
                $(this).parent().siblings().children().removeClass('laydate_click');
                Dates.stopmp(ev);
    ...
    Dates.on(as.otoday, 'click', function(){
        var now = new Date();
        Dates.creation([now.getFullYear(), now.getMonth() + 1, now.getDate()],false);
    });
    ...
    Dates.on(as.ok, 'click', function(){
        if(Dates.valid){
            Dates.creation([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]],false);
        }
    });
...
// $(function () {
//     $("#laydate_hmsno span").css("background","#000")
// })
