...
<body>
	...
	<div id="divMain">
		<div id="divTitle">
			<span id="spanTitle">编辑预案参数页面</span>
		</div>
		...
		<div id="divBody">
			<table id="tableForm">
				<tr>
					...
					<td class="tdInput">
						<input class="inputClass" type="text" name="yamc" id="yaName" value="${yacs.yamc }">
					</td>
					<td class="tdError">
				...
				<tr>
					...
					<td>
						<input class="inputClass" type="text" name="yams" id="yaDesc" value="${yacs.yams }">
					</td>
					<td class="tdError">
					...
					<td>
						<select class="inputClass" id= "Check_Yazl" name="yazl" style="width: 240px;height:32px; border:1px solid #7f9db9;">
	           				...
	           				<c:forEach items= "${Ya_kinds }" var="y">
	           					<option value="${y.typeSerialNo }" <c:if test="${y.typeSerialNo eq yacs.yazl }">selected="selected"</c:if> >${y.typeDesc }</option>
	           				</c:forEach>
	           			</select> 
					</td>
					<td class="tdError">
			...
			<input type="hidden" name="id" value="${ yacs.id}">
			<input type="hidden" name="Check_Yamc" value="${ Check_Yamc}">
			<input type="hidden" name="Check_Yazl" value="${ Check_Yazl}">
			<input type="hidden" name="Check_Yadj" value="${ Check_Yadj}">
...
<script type="text/javascript">
	...
	//Controller中返回的数据
	var yacs = $.parseJSON('${yacs_json}');//预案参数
	var message = '${message}';//返回的错误信息
	var yalb = $.parseJSON('${Ya_lbs}');//获取预案类别
	var bjlx = $.parseJSON('${list_dicList_json}');//报警类型
	//文档加载完之后执行
	$(function(){
		...
		if(typeof(zl) != undefined){
			...
			for(var y in yalb){
				if(yalb[y].SUPERID==zl){
					if(yacs.yadj==yalb[y].ID){
						$yalb.append("<option class='newOption' selected='selected' value='"+yalb[y].ID+"'>"+yalb[y].NAME+"</option>");
					}else{
			...
			if(zl=='1'){
				for(var b in bjlx){
					if(bjlx[b].typeCode=='BKDL1'){
						if(yacs.bjlx==bjlx[b].typeSerialNo){
							$bjlx.append("<option class='newOption' selected='selected' value='"+bjlx[b].typeSerialNo+"'>"+bjlx[b].typeDesc+"</option>");
						}else{
	...
	function doSubmit(){
		...
		if(verifyResult1 && verifyResult2 && verifyResult3 && verifyResult4&&verifyResult5){
			//提交表单
			document.forms[0].action="yagl/updateYa.do";
