	...
	<body>
		...
		<div class="content">
			<div class="content_wrap">
				<fieldset style="margin: 0px 0px 0px 0px;">
					...
					<div class="slider_body">
				        <div class="slider_selected_left">
				            <span id="csx_span" class="layer_span" style="color:#900b09">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;超时分界线：</span>
				        </div>
						<div class="slider_selected_right dropdown dropdowns" onclick="slider(this)">
							<input class="input_select xiala" id="csx_downlist" readonly="readonly" type="text" value="10秒"/>
							<input type="hidden" id="csx_select" name="property" value="10"/>
							<div class="ul">
							    <%
			                		for(int i=10;i <= 120;){
			                	%>
				                    	<div class="li" data-value="<%=i %>" onclick="sliders(this)"><%=i %>秒</div>
				                <%
				                		i = i + 10;
				                    }
				                %>
				...
				<fieldset style="margin: 20px 0px 0px 0px;">
			      	<legend style=" color:#FF3333">监测结果（此处所涉及的数据量不包含补传的数据）</legend>
		...
		<script type="text/javascript">
			$(function(){
			    ...
			    //超时分界线
			    showTip("#csx_span", "如果上传时间减去通过时间大于超时分界线，则视为上传超时。即超时定义为：上传时间 - 通过时间 > 超时分界线。");
			});
			...
			function doSearch() {
				...
				//超时分界线
				var csx = parseInt($("#csx_select").val());
				...
				//提交
				$.ajax({
					url:'<%=basePath%>/sjjk/sjcsqkjc.do?' + new Date().getTime(),
					method:"post",
					data:{kssj:kssj, jssj:jssj, jcd:jcdStrs, csbz:csx},
			...
			function doExport(){
				if($("#table1 tbody tr[id='tr_result']").length >= 1){
					var url = "${pageContext.request.contextPath}/sjjk/exportSjcsqkjcExcel.do";
