	...
	<body>
			...
			<div class="content_wrap">
				<fieldset style="margin: 0px 0px 0px 0px;">
					...
					<div class="slider_body">
						...
						<div id="khbm_allselect" class="slider_selected_right dropdown dropdown_all">
							<div class="input_select xiala">
								<div id="khbm_downlist" class='multi_select'>
									<input type="hidden" id="khbm_select" value=""/>
									<a class="xiala_duoxuan_a"></a>
								</div>
							</div>
						</div>
					</div>
					<div class="button_wrap clear_both" style="height: 80px;">
		...
		<script type="text/javascript">
			$(function(){
				...
				//考核部门
				var deptsList = jQuery.parseJSON('${deptsListOfJson}');
				var value = []; 
				var data = [];
				for(var i=deptsList.length-1;i >= 0;i--){
					value.push(deptsList[i].deptNo);
					data.push(deptsList[i].deptName);
				}
				$('#khbm_downlist').MSDL({
					'value': value,
				    'data': data
			...
			function doSearch() {
				...
				//提交
				$.ajax({
					url:'<%=basePath%>/sjjk/ywczqkjc.do?' + new Date().getTime(),
					method:"post",
					data:{kssj:kssj, jssj:jssj, khbm:khbm},
			...
			function doExport(){
				if($("#table1 tbody tr[id='tr_result']").length >= 1){
					var url = "${pageContext.request.contextPath}/sjjk/exportYwczqkjcExcel.do";
			...
			//显示信息详情
		  	function alertInfo(bm, zs, jss, css, wqs, jsl){
		  	    var ms = "   部门：" + bm + "\n   总数：" + zs + "\n及时数：" + jss + "\n超时数：" + css + "\n未签收：" + wqs + "\n及时率：" + jsl;
		  		alert(ms);
		  	}
		</script>
