...
<body>
						...
						<tbody>
							<c:forEach var="instructionSign" items="${pageResults.items }" varStatus="status">
								...
								<tr>
									...
									<td>${instructionSign.fkztmc}</td>
									<td>
										<a onclick="insDetail('${instructionSign.id}')" style="font-size: 15px;">详情</a>
										<c:if test="${instructionSign.qszt eq '1' and instructionSign.fkzt eq '0'}">
											|&nbsp;<a onclick="feedback('${instructionSign.id}')" style="font-size: 15px;">反馈</a>
										</c:if>
									</td>
								</tr>
							</c:forEach>
	...
	<script type="text/javascript">
		var list_url = "queryInsSignForFk.do";
		//刷新..按钮
		function refresh() {
			layer.msg("刷新完成！");
			doSearch();
