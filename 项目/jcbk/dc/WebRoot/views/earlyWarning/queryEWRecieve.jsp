...
<html lang="zh-CN">
	...
	<body>
		...
		<div id="divTitle">
			<span id="spanTitle">当前位置：预警管理&gt;&gt;预警签收查询</span>
		</div>
		<div class="content">
		        	...
		        	<div class="slider_body">
			            ...
			            <div class="slider_selected_right dropdown dropdowns" onclick="slider(this)">
			                ...
			                <div class="ul">
			                	<div class="li" data-value="" onclick="sliders(this)">==全部==</div>
							    <c:forEach items= "${dicList}" var="dic">
			            	  		<c:if test="${dic.typeCode eq 'YJQRZT' }">
				                    	<div class="li" data-value="${dic.typeSerialNo}" onclick="sliders(this);">${dic.typeDesc}</div>
	...
	<!-- 								<td width="120" align="center">预警接收部门</td> -->
									<td width="100" align="center">预警签收部门</td>
							...
							<tbody>
								<c:forEach var="ewrecieve" items="${pageResults.items }" varStatus="status">
									<c:set var="ewarning" value="${ewrecieve.ewarning }" />
									<c:set var="d" value="${ewarning.dispatched }" />
									<tr>
										...
										<td style="color: green;">
											<c:forEach items= "${dicList}" var="dic">
						            	  		<c:if test="${dic.typeCode eq 'BKDL' }">
							                    	<c:if test="${dic.typeSerialNo eq ewrecieve.bjdl }">
							                    		${dic.typeDesc }
							                    	</c:if>
						            	  		</c:if> 
										 	</c:forEach>
										...
										<td style="color: green;">
											<c:if test="${ewrecieve.bjdl eq '1'}">
												<c:forEach items= "${dicList}" var="dic">
							            	  		<c:if test="${dic.typeCode eq 'BKDL1' }">
								                    	<c:if test="${dic.typeSerialNo eq ewrecieve.bjlx }">
								                    		${dic.typeDesc }
								                    	</c:if>
							            	  		</c:if> 
											 	</c:forEach>
											</c:if>
											<c:if test="${ewrecieve.bjdl eq '2'}">
												<c:forEach items= "${dicList}" var="dic">
	...
	<!-- 									<td>${ewrecieve.bjbmmc}</td> -->
										<td>${ewrecieve.qrdwdmmc}</td>
										<td>${ewrecieve.qrr}</td>
										<td>${ewrecieve.qrztmc}</td>
										<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${ewrecieve.qrsj}" /></td>
										<td>
											<a onclick="eWRecieveDetail('${ewrecieve.qsid}');" style="font-size: 15px;">详情</a>
											<c:if test="${ewrecieve.bjdl eq '1' and isCity eq '1' and ewrecieve.qrzt eq '1' and ewrecieve.by1 eq '0'}">
												|&nbsp;<a onclick="sendInstruction('${ewrecieve.qsid}');" style="font-size: 15px;">下发指令</a>
											</c:if>
										</td>
									</tr>
								</c:forEach>
	...
	<script type="text/javascript">
		...
		var list_url = "earlyWarning/queryEWRecieve.do";
		//搜索..按钮
		function doSearch() {
			//检查并组装时间
			if(!changeTime()){
		...
		function eWRecieveDetail(qsid){
			var url = "${pageContext.request.contextPath}/earlyWarning/loadEWRecieveDetail.do?qsid=" + qsid;
