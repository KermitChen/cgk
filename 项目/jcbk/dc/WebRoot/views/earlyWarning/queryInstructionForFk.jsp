...
<!-- 	        	</div> -->
					...
					<table>
						<thead>
							<tr>
								...
								<td width="150" align="center">反馈时间</td>
						...
						<tbody>
							<c:forEach var="insSign" items="${pageResults.items }" varStatus="status">
								...
								<tr>
									...
									<td>${insSign.fkrmc}</td>
									<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${insSign.fksj}" /></td>
									<td>${insSign.qsztmc}</td>
									<td>${insSign.fkztmc}</td>
									<td>
										<a onclick="insDetail('${insSign.id}')">详情</a>
										<c:if test="${insSign.qszt eq '1' and insSign.fkzt eq '0'}">
											|&nbsp;<a onclick="feedback('${insSign.id}')">反馈</a>
										</c:if>
									</td>
								</tr>
							</c:forEach>
	...
	<script type="text/javascript">
		...
		var list_url = "earlyWarning/queryInstructionForFk.do";
		//搜索..按钮
		function doSearch() {
			//检查并组装时间
			if(!changeTime()){
		...
		function feedback(id){
			var url = "${pageContext.request.contextPath}/earlyWarning/loadInsFeedback.do?id="+id;
		    layer.open({
		           type: 2,
		           title: '指令签收窗口',
		           shadeClose: false,
