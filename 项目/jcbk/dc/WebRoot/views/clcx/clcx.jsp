...
<body>
	...
	<div id="divTitle">
		<span id="spanTitle">当前位置：车辆综合查询&gt;&gt;车辆轨迹查询</span>
	</div>
    <div class="content" style="height: 700px">
    	<div class="content_wrap">
    	 	<form id="form1" name="form1" action="<%=basePath%>clcx/clcx.do" method="post">
	       ...
	       <div class="slider_body hidden_div" <c:if test="${isHiddenDiv eq '0'}">style="display:none;"</c:if>>
	            ...
	            <div class="slider_selected_right dropdown dropdowns recongnized_div" id="cp_3" onclick="slider(this)">
	            <c:choose>
	            	<c:when test="${empty unrecognizedFlag }">
		                <input class="input_select xiala" id="reco_xiala" readonly="readonly" type="text" value="==全部=="/> 
	            	</c:when>
	            	<c:when test="${unrecognizedFlag eq '1'}">
	            		<input class="input_select xiala" id="reco_xiala" readonly="readonly" type="text" value="已识别"/> 
	            	</c:when>
	            	<c:when test="${unrecognizedFlag eq '0'}">
	            		<input class="input_select xiala" id="reco_xiala" readonly="readonly" type="text" value="未识别"/> 
	            	</c:when>
	            	<c:otherwise>
	            		<input class="input_select xiala" id="reco_xiala" readonly="readonly" type="text" value="==全部=="/> 
	            	</c:otherwise>
	            </c:choose>
	                 <input type="hidden" id="unrecognizedFlag" name="unrecognizedFlag" value="${unrecognizedFlag }" />
	                <div class="ul"> 
	                 		<div class="li" data-value="11" onclick="sliders(this)">==全部==</div>
		                    <div class="li" data-value="1" onclick="sliders(this)">已识别</div>
		                    <div class="li" data-value="0" onclick="sliders(this)">未识别</div>
	                </div> 
	            </div>
	        </div>
	        ...
	        <div class="pg_result">
	            	...
	            	<c:forEach items="${pageResult.items }" var="s" varStatus="c">
		            	<div class="detail_div">
		            		...
		            		<div class="content_div">
		            			...
		            			<div class="text_div"><c:if test="${not empty jcdMap[s.jcdid] }">${jcdMap[s.jcdid] }</c:if><c:if test="${empty jcdMap[s.jcdid] }">${s.jcdid }</c:if></div>
		            		</div>
		            	</div>
	            	</c:forEach>
	            </div>
	            	 ...
	            	 <table>
		                ...
		                <tbody>
		                    <c:forEach items="${pageResult.items }" var="s" varStatus="c">
		                        ...
		                        <td>${s.sbsj }</td>
		                        <td>${s.scsj }</td>
		                        <td><c:if test="${not empty jcdMap[s.jcdid] }">${jcdMap[s.jcdid] }</c:if><c:if test="${empty jcdMap[s.jcdid] }">${s.jcdid }</c:if></td>
		                        <td>${s.cdid }</td>
		                        <td>${s.sd }</td>
	           ...
	           <c:if test="${!empty pageResult.items}">
					    	...
					    	<div class="mess_div ${c.index }" <c:if test="${c.index ne 0 }">style="display:none;"</c:if>>
					    		<fieldset style="-moz-border-radius:8px;border:#D2691E 1px solid;">
			                		...
			                		<table id="three_first_table">
			                			...
			                			<tr>
			                				<td colspan="2">
			                					&nbsp;&nbsp;&nbsp;监测点:&nbsp;&nbsp;<span><c:if test="${not empty jcdMap[s.jcdid] }">${jcdMap[s.jcdid] }</c:if><c:if test="${empty jcdMap[s.jcdid] }">${s.jcdid }</c:if></span>
			                				</td>
			                			</tr>
			                		</table>
		                		</fieldset>
            	...
            	<input type="hidden" value="${pageResult.totalPageCount }" id="totalPageCount">
	        </div>
			</form>
        </div>
    </div>
	...
	<script type="text/javascript">
		var list_url = "<%=basePath%>clcx/clcx.do";
		//根据页号查询
		...
		function doSubmit(){
			...
			var cphid = $.trim($("#cphid").val());
			var recongnizeFlag = $("#unrecognizedFlag").val();
			if(recongnizeFlag == "0" && isNotEmpty(cphid)){
				$("#errSpan").text("错误提示：未识别车辆不可以输入车牌号！");
				return;
			}
			var style = $("input[name='showStyle']:checked").val();
			if(style == 'show_style3' && (cphid.length > 9 || cphid.indexOf(";") != -1) ){
				layer.msg('只能查询一辆车的轨迹！');
				return;
			}
			layer.load();
		...
		function changeTime(){
			...
			if(cxlb == "1"){//按天
				...
				var flag = compareMoment(now,day,'YYYY-MM-DD');
				if(flag == '1'){
