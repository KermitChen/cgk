...
<body>
	...
	<div id="divTitle">
		<span id="spanTitle">当前位置：红名单管理&gt;&gt;红名单申请</span>
	</div>
                        	...
                        	<tr>
                        		...
                        		<td>
                        			<c:if test="${r.jlzt eq '001'}">
	                        			<a id="detail${id }" href="javascript:doCancelHmd('suspend','${pi.id}','${r.id }')">取消申请</a>
	                        			|
                        			</c:if>
                        			<c:if test="${r.jlzt eq '002' and r.jjhomdCx.jlzt ne '001'}">
	                        			<a id="detail${id }" href="javascript:doRevokeHmd(${r.id })">撤销红名单</a>
	                        			|                        			
                        			</c:if>
                        			<c:if test="${r.jlzt eq '002' and r.jjhomdCx.jlzt eq '001'}">
	                        			<a id="detail${id }" href="javascript:doCancelRevokeHmd('suspend','${pi2.id}','${r.id }')">取消撤销</a>
	                        			|                        			
                        			</c:if>
                        			<c:if test="${r.rwzt eq '01' }">
	                        			<c:if test="${!empty task.id }">
	                        				<a id="detail${id }" href="javascript:doDetail(${r.id },${task.id })">详情</a>
	                        			</c:if>
	                        			<c:if test="${empty task.id }">
                        			...
                        			<c:if test="${r.rwzt eq '02' }">
	                        			<c:if test="${!empty task2.id }">
	                        				<a id="detail${id }" href="javascript:doDetail(${r.id },${task2.id })">详情</a>
	                        			</c:if>
	                        			<c:if test="${empty task2.id }">
	                        				<a id="detail${id }" href="javascript:doDetail(${r.id })">详情</a>
	                        			</c:if>
                        		...
                        		<td align="center">
                        			...
                        			<c:if test="${r.rwzt eq '02' }">
										<a class="trace"  pid="${pi.id }" pdid="${pi.processDefinitionId}" title="点击查看流程图">${task2.name }</a>
									</c:if>
								</td>
                        	</tr>
                        </c:forEach>
...
<script type="text/javascript">
	...
	//导出excel
	function doExport(){
		var url = "${pageContext.request.contextPath}/JjHmd/exportHmdExcel.do";
	...
	//取消申请   红名单
	function doCancelHmd(state,processInstanceId,id){
		layer.confirm('您确定取消申请红名单？', {
  			btn: ['确定','取消'] //按钮
		},function(){
			var url ="${pageContext.request.contextPath}/JjHmd/cancelHmd.do";
			$.ajax({
				data:{hmdId:id,state:state,processInstanceId:processInstanceId},
				type:'POST',
				dataType:"text",
				url:url,
				error:function(message){
	...
	//取消撤销红名单的申请
	function doCancelRevokeHmd(state,processInstanceId,id){
		layer.confirm('您确定取消撤销红名单？', {
  			btn: ['确定','取消'] //按钮
		},function(){
			var url ="${pageContext.request.contextPath}/JjHmd/cancelRevokeHmd.do";
			$.ajax({
				data:{hmdId:id,state:state,processInstanceId:processInstanceId},
				type:'POST',
				dataType:"text",
