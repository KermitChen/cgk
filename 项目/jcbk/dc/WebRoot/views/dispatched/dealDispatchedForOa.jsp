...
<body>
	<div class="content_oasp" style="height: 1200px;">
    	<div class="content_wrap" style="width:560px;">
    		<div id="print">
                <h1 align="center" style="font-weight: bold"><span style='font-size:24.0pt;font-family:黑体'>深圳市公安局车辆布控审批表</span></h1>
...
<!--                         <tr> -->
<!--                         	<td style="border:solid windowtext 2.25pt;width: 70px;">布控范围类型</td> -->
<!--                         	<td colspan="3" id="bkfwlx" style="border:solid windowtext 2.25pt;width: 70px;text-align: left"></td> -->
<!--                         </tr> -->
                        <tr>
                        	<td style="border:solid windowtext 2.25pt;width: 22%;">
	        ...
	        <form name="form" action="" method="post">
		        <div id="operateDiv" class="button_wrap1" style="width:100%;height:50px;font-size: 14px;">
						...
						<div class="slider_body" style="width:450px;">
				            ...
				            <div class="slider_selected_right" style="width:400px" id="spcz">
			                    ...
			                    <c:if test="${isDeal eq true }">
				                        <input type="radio" name="spcz" value="0"/>不同意&nbsp;&nbsp;
				                        <c:if test="${isComplete eq true }">
				                        	<input type="radio" name="spcz" value="4"/>同意(结束审批)
				                        </c:if>
			                    </c:if>
			                </div>
				        </div>
				    ...
				    <c:if test="${dispatched.zjbk eq '1' && (user.position eq '81' || user.position eq '91')}">
						<div class="slider_body" style="width:380px;">
				        	...
				        	<div class="slider_selected_right" style="width:300px" id="spcz">
				        		<input type="radio" name="spcz" value="1" checked="checked"/>确认布控(并报备领导)&nbsp;&nbsp;
			            	</div>
				        </div>
				    </c:if>
					...
					<c:if test="${dispatched.zjbk eq '0' }"><button class="button_blue" style="width: 100px;" type="button" onclick="agreeBk();">提交</button></c:if>
					<c:if test="${dispatched.zjbk eq '1' && user.position ne '81' && user.position ne '91'}">
						<c:if test="${leaders ne '[]' }">
							<button class="button_blue" style="width: 100px;" type="button" onclick="dealReport(true)">呈送上级</button>
						</c:if>
						<button class="button_blue" style="width: 100px;" type="button" onclick="dealReport(false)">结束报备</button>
					</c:if>
					<c:if test="${dispatched.zjbk eq '1' && (user.position eq '81' || user.position eq '91')}">
						<c:if test="${leaders ne '[]' }">
							<button class="button_blue" style="width: 100px;" type="button" onclick="dealReport1(true);">报备上级</button>
						</c:if>
					</c:if>
		        	<button class="button_blue" style="width: 88px;" type="button" onclick="toPrint();">打印</button> 
		        	<button class="button_blue" style="width: 88px;" type="button" onclick="toClose();">关闭</button>
		        </div>
	        </form>
    </div>
    <div class="mask"></div>
	...
	<script>
			...
			if('${dispatched.zjbk}' == '1'){
				...
				for(var i=0;i < disReportList.length;i++){
					...
					if(bbrjs == '2' && bbrjs1 == zw && bbrjsNum == zwNum){
						...
						$("#kldAdvice").val(disReportList[i].fknr);
						...
						$("#cldAdvice").val(disReportList[i].fknr);
						...
						$("#jldAdvice").val(disReportList[i].fknr);
		...
		function agreeBk(){
			...
			if($('input:radio[name=spcz]:checked').val() == "1"){
				completeForOa('${task.id }','${task.processInstanceId }','${passKey }', '1', adviceMsg, '${conPath }',false,'${dispatched.bkid }', '${user.loginName }');
			} else if($('input:radio[name=spcz]:checked').val() == "4"){
				completeForOa('${task.id }','${task.processInstanceId }','${passKey }', '4', adviceMsg, '${conPath }',false,'${dispatched.bkid }', '${user.loginName }');
			} else{
				completeForOa('${task.id }','${task.processInstanceId }','${passKey }', '0', adviceMsg, '${conPath }',false,'${dispatched.bkid }', '${user.loginName }');
			}
		}
		...
		function dealReport(czjg){
			...
			$.ajax({
				url: '<%=basePath %>dispatched/dealReportForOa.do?login=1',
				method: "post", 
