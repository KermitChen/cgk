...
<html>
	<head>
		...
		<meta http-equiv="description" content="深圳市公安局缉查布控系统-修改角色权限">
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				//触发角色类型单选，赋默认值
				$("<%="#" + roleObj.getRoleType() %>").trigger("click");//
				...
				$("#saveBt").click(function() {
					...
					//提交
					$.ajax({
						url:'<%=basePath%>/role/updateRole.do?' + new Date().getTime(),
						method:"post",
						data:{id:<%=roleObj.getId() %>, roleName:roleName, roleType:roleType, selectFun:selectFun},
	...
	<body>
		...
		<fieldset style="margin: 0px auto">
			<legend style="color:#FF3333">修改角色</legend>
		        ...
		        <div class="slider_selected_right" style="left: 90px;">
		        	<div class="img_wrap">
		            	<div class="select_wrap select_input_wrap">
		                	<input id="roleNameAdd" type="text" class="slider_input" value="${roleObj.roleName}">
					...
					<div class="ul">
						...
						<%
							...
							for(int i=dicList.size()-1;i >= 0;i--){
								...
								//if("1001".equals(dic.getTypeCode()) && Integer.parseInt(dic.getTypeSerialNo().substring(0, 2)) < Integer.parseInt((userObj.getPosition() == null || "".equals(userObj.getPosition().trim())?"0":userObj.getPosition().trim().substring(0, 2)))){
						...
						%>
					                <div id="<%=dic.getTypeSerialNo() %>" class="li" data-value="<%=dic.getTypeSerialNo() %>" onclick="sliders(this)"><%=dic.getTypeDesc() %></div>
						<%
								}
							}
						%>
